...
const MARATHON_TICKS = 120000;

const FILES = [
  'floatingText.js', 'pathfinding.js', 'spatial.js', 'sprites.js', 'visual-polish.js', 'audio.js', 'particles.js', 'effects.js',
...
function ctx2d() {
  return {
    ...
    closePath: noop, moveTo: noop, lineTo: noop, arc: noop, fill: noop, stroke: noop, fillRect: noop, strokeRect: noop,
...
function el(id = '') {
  return {
    ...
    style: {}, textContent: '', innerHTML: '', value: '', dataset: {},
    addEventListener: noop, removeEventListener: noop, click: noop, appendChild: (c) => c,
    querySelector: (s) => el(s), querySelectorAll: () => [],
    getContext: (t) => (t === '2d' ? ctx2d() : null), width: 64, height: 64,
  };
}
...
function boot(seed) {
  ...
  const sb = {
    ...
    document: { getElementById: (id) => ids.get(id) || mk(id), querySelector: (s) => ids.get(s?.slice(1)) || el(), querySelectorAll: () => [], addEventListener: noop, createElement: () => el(), hidden: false },
    canvas: cvs,
  };
...
function loadOnce() {
  ...
  const exp = vm.runInContext(`${code}\n({ Game, Cheats, MetaProgress, SpriteGen, Pathfinding, Spatial, GameModes, AdvancedDifficulty, Legacy, Chronicles })`, vm.createContext(sb));
...
const STRATS = [
  ['begin_day_spam', (Gm) => { if (Gm.getState().timeOfDay === 'night') Gm.beginDayPhase(true); }],
  ['begin_day_early', (Gm, t) => { if (Gm.getState().timeOfDay === 'night' && t % 4 === 0) Gm.beginDayPhase(true); }],
  ['pause_at_dusk', (Gm) => { const s = Gm.getState(); if (s.timeOfDay === 'day' && s.waveProgress > 0.85) Gm.togglePause(); }],
  ['levy_spam', (Gm) => { const s = Gm.getState(); if (s.hasCourier && !s.courierUsedThisWave) Gm.sendCourierMessage('levy'); }],
  ['decree_spam', (Gm) => { const s = Gm.getState(); if (s.hasCourier && !s.courierUsedThisWave) Gm.sendCourierMessage('decree'); }],
  ['spy_steal', (Gm) => { if (!Gm.getState().spyUsedThisWave) Gm.executeSpyAction('steal'); }],
  ['spy_scout', (Gm) => { if (!Gm.getState().spyUsedThisWave) Gm.executeSpyAction('scout'); }],
  ['rally_spam', (Gm) => { if (Gm.getState().tactical >= 4) { Gm.selectAbility('rally'); Gm.handleClick(400, 400); } }],
  ['tp_burst', (Gm, t) => { if (t % 400 === 0) Gm.applyCheatEffect('tp', 40); }],
  ['deploy_night', (Gm, t) => { const s = Gm.getState(); if (s.timeOfDay === 'night' && s.canDeploy && s.tactical >= 3) { Gm.selectDeploy('footman'); Gm.handleClick(200 + (t % 10) * 20, 500); } }],
  ['build_night', (Gm, t) => { const s = Gm.getState(); if (s.timeOfDay === 'night' && s.tactical >= 6) { Gm.selectBuild('wall'); Gm.handleClick(300, 450); } }],
  ['hunt_flip', (Gm, t) => { if (t % 300 === 0) Gm.toggleGlobalHunt(); }],
  ['wwe_cheat', (Gm, t, C) => { if (t === 1) C.submit('Austin 3:16'); }],
  ['doom_cheat', (Gm, t, C) => { if (t === 1) C.submit('Doomslayer'); }],
  ['morale_cheat', (Gm, t) => { if (t % 500 === 0) Gm.applyCheatEffect('morale'); }],
  ['iddqd', (Gm, t, C) => { if (t % 200 === 0) C.submit('iddqd'); }],
  ['rosebud', (Gm, t, C) => { if (t % 350 === 0) C.submit('rosebud'); }],
  ['highlander', (Gm, t, C) => { if (t % 250 === 0) C.submit('there can be only one'); }],
  ['reinforce', (Gm) => { const s = Gm.getState(); if (s.timeOfDay === 'day' && s.tactical >= 10 && s.canDeploy) { Gm.selectAbility('reinforce'); Gm.handleClick(400, 400); } }],
  ['save_export', (Gm, t) => { if (t % 800 === 0 && Gm.exportGameState) Gm.exportGameState(); }],
  ['passive', () => {}],
];
...
function sim(runId, maxTicks, fastNight) {
  ...
  for (let t = 0; t < maxTicks; t++) {
    ...
    if (t % 8 === 0 && pre.state === 'playing' && pre.timeOfDay === 'day') Game.applyCheatEffect('clear_enemies');
    const t0 = Date.now();
    try {
    ...
    if (post.timeOfDay === 'night' && pre.timeOfDay === 'day') {
      ...
      if (tpNight !== null && tpGain > 150) issues.push({ runId, code: 'DOUBLE_TP', gain: tpGain, name });
  ...
  if (avgUpdateMs > 8) issues.push({ runId, code: 'SLOW_UPDATE', ms: +avgUpdateMs.toFixed(2), name });
  return { runId, name, maxWave: Math.max(maxWave, fin.wave), issues, reached1000: fin.wave >= 1000, nights, avgUpdateMs: +avgUpdateMs.toFixed(2) };
}
...
for (let m = 0; m < 3; m++) {
  ...
  results.push(r);
  issues.push(...r.issues);
  console.log(` Marathon ${m}: wave ${r.maxWave} avg ${r.avgUpdateMs}ms${r.reached1000 ? ' *** 1000+ ***' : ''}`);
}
...
const report = {
  runs: results.length, elapsedSec: +elapsed,
  bestWave: Math.max(...waves), avgWave: +(waves.reduce((a, b) => a + b, 0) / waves.length).toFixed(1),
