...
const Achievements = (() => {
  ...
  let session = {
    kills: 0, elitesKilled: 0, vetUpgrades: 0, maxWave: 0,
    deployedTypes: new Set(), abilitiesUsed: new Set(),
    buildingsCompleted: new Set(), enemyFirst: new Set(),
    flags: new Set(), currentDiff: 'normal', currentMisses: 0,
    doomKills: 0, spyThisWave: 0, academyTypes: new Set(),
    crossoverFieldMax: {}, crossoverFieldNow: {},
    garrisonCount: 0, advancedMods: 0,
    maxCrossoverFactions: 0, maxSynergyCount: 0,
  };

  const CROSSOVER_TABS = CROSSOVER_ACH_FACTIONS.map(f => ({ id: f.key, label: f.label, cat: f.cat }));
  ...
  const CATS = [
    ...
    { id: 'crossovers', label: 'Crossovers', crossover: true },
    { id: 'secrets', label: 'Secrets/Meta', cats: ['secrets', 'meta'] },
  ];

  const VANILLA_CATS = new Set(['waves', 'combat', 'army', 'build', 'specialists', 'difficulty', 'tactics', 'economy', 'vanilla']);

  function load() {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      if (!raw) { migrateFromV2(); return; }
  ...
  function save() {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        ...
        lifetimeSpy, lifetimeCourier: [...lifetimeCourier],
        lifetimeEnemyKills, lifetimeKills, lifetimeWavesCleared,
        crossoverKills, crossoverAbilities, crossoverMastery,
        doomLifetimeKills, doomMaxAliveWaves, hiddenMilestones,
      }));
    } catch (_) { /* ignore */ }
  }
  ...
  function applyReward(ach) {
    ...
    if (r.type === 'tp' && typeof Game !== 'undefined' && Game.isPlaying?.()) {
      Game.creativeAddTp?.(r.amount) || (Game.getState && (() => {
  ...
  function pushRecent(ach) {
    recentUnlocked = [{ id: ach.id, name: ach.name, tier: ach.tier, at: Date.now() }, ...recentUnlocked.filter(r => r.id !== ach.id)].slice(0, 12);
  }
  ...
  function tryUnlock(id) { return unlock(id); }

  function checkMilestones() {
    const count = effectiveUnlockCount();
    const preMeta = [...unlocked].filter(i => i !== META_316_ID && i !== CLUB_316_ID && i !== MILLENNIUM_ID).length;
    if (preMeta >= 316) {
  ...
  function showNextToast() {
    if (!toastQueue.length) { toastActive = null; return; }
    toastActive = toastQueue.shift();
    const el = document.getElementById('achievement-toast');
    if (!el) { toastActive = null; return; }
    ...
    if (!nameEl || !descEl) { toastActive = null; return; }
    const c = getCount();
    ...
    if (rewardEl) {
      ...
      rewardEl.textContent = rw?.type === 'tp' ? `Reward: +${rw.amount} TP (Creative)` : rw?.type === 'badge' ? '316 Club badge earned!' : '';
  ...
  function evaluateRule(rule, ctx = {}) {
    ...
    switch (kind) {
      ...
      case 'session_deploy_all': {
        const all = ['footman','archer','mage','cavalry','healer','knight','sapper','builder','courier','general'];
      ...
      case 'build':
        return rest[0] === 'any' ? session.buildingsCompleted.size > 0 : session.buildingsCompleted.has(rest[0]);
      case 'walls':
      ...
      case 'settlement_chain':
        return (ctx.hamletCount ?? 0) >= parseInt(rest[0], 10) && (ctx.guildCount ?? 0) >= parseInt(rest[1], 10);
      case 'support_pair':
        return session.buildingsCompleted.has('medical_tent') && session.buildingsCompleted.has('mess_hall');
      case 'ability':
      ...
      case 'diff_flawless':
        return session.currentDiff === rest[0] && session.maxWave >= parseInt(rest[1], 10) && session.currentMisses === 0;
      case 'advanced_mods':
      ...
      case 'unlocked_count': {
        ...
        const pre = [...unlocked].filter(i => i !== META_316_ID && i !== CLUB_316_ID && i !== MILLENNIUM_ID).length;
        return pre >= need;
      }
      ...
      case 'crossover_all': {
        const f = rest[0];
        if (f === 'wwe') return Object.keys(WweDefs).every(id => MetaProgress.getWweRecruited().includes(id));
        const ids = getFactionOperativeIds(f);
        return ids.length > 0 && ids.every(id => MetaProgress.getCrossoverRecruited().includes(id));
      }
      case 'crossover_field':
        return (session.crossoverFieldMax[rest[0]] || 0) >= parseInt(rest[1], 10);
      case 'crossover_wave': {
        const need = parseInt(rest[1], 10);
        const minField = rest[2] != null
          ? parseInt(rest[2], 10)
          : (rest[0] === 'wwe' ? 3 : 2);
      ...
      case 'crossover_grand_slam_all':
        return CROSSOVER_ACH_FACTIONS.every(f => {
          ...
          if (f.key === 'wwe') return Object.keys(WweDefs).every(id => MetaProgress.getWweRecruited().includes(id));
          if (f.key === 'doom') return true;
          return ids.length > 0 && ids.every(id => MetaProgress.getCrossoverRecruited().includes(id));
        });
      case 'multiversal':
        return Math.max(ctx.crossoverFactionCount ?? 0, session.maxCrossoverFactions) >= parseInt(rest[0], 10);
      case 'doom_alive':
  ...
  function onEvent(type, data = {}) {
    switch (type) {
      ...
      case 'wave_complete': {
        ...
        session.currentMisses = data.misses ?? 0;
      ...
      case 'wwe_recruit':
        ...
        if (Object.keys(WweDefs).every(id => MetaProgress.getWweRecruited().includes(id))) tryUnlock('wwe_grand_slam');
        break;
      ...
      case 'crossover_recruit':
        MetaProgress.recordCrossoverRecruit(data.crossoverId);
        {
          ...
          if (def?.faction) {
            ...
            if (ids.every(id => MetaProgress.getCrossoverRecruited().includes(id))) tryUnlock(`${def.faction}_grand_slam`);
          }
        }
        break;
  ...
  function matchesTab(ach, tabId) {
    ...
    if (tab.cats) return tab.cats.includes(ach.cat) || (tabId === 'vanilla' && VANILLA_CATS.has(ach.cat));
  ...
  function renderPanel() {
    ...
    if (subTabsEl) {
      if (activeCat === 'crossovers') {
        ...
        subTabsEl.innerHTML = CROSSOVER_TABS.map(ct => `
    ...
    if (recentEl) {
      recentEl.innerHTML = recentUnlocked.length
        ? recentUnlocked.map(r => {
          ...
          const tier = badge ? `<span class="ach-recent-tier tier-${badge.cls}">${badge.label}</span>` : '';
    ...
    else if (tierFilter !== 'all') filtered = filtered.filter(a => a.tier === tierFilter || (tierFilter === 'meta' && a.meta));
    ...
    grid.innerHTML = filtered.map(a => {
      ...
      const badgeHtml = badge ? ` <span class="ach-tier-badge tier-${badge.cls}">${badge.label}</span>` : '';
    ...
    grid.querySelectorAll('.ach-card.unlocked').forEach(card => {
      card.addEventListener('click', () => {
        ...
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text).then(() => showShareFlash(card)).catch(() => {});
        }
      });
    });
  }
  ...
  return {
    init, onEvent, tryUnlock, getAll, getCount, renderPanel, togglePanel,
    resetSession, checkAll, evaluateRule, updateTopBar, hasClub316,
    CLUB_316_ID, META_316_ID, MILLENNIUM_ID, TARGET,
  };
})();