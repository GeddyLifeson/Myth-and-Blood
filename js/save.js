...
const SaveManager = (() => {
  ...
  function wrapSnapshot(snap, label) {
  ...
  function saveToSlot(index, label) {
    ...
    const store = loadStore();
    while (store.slots.length < SLOT_COUNT) store.slots.push(null);
    store.slots[index] = wrapSnapshot(snap, label || `Wave ${snap.wave}`);
  ...
  function deleteSlot(index) {
    const store = loadStore();
    if (!store.slots?.[index]) return false;
  ...
  function renderPauseSlots() {
    ...
    host.innerHTML = slots.map(s => {
      ...
      return `<div class="save-slot" data-slot="${s.index}">
        <div class="save-slot-info"><strong>Slot ${s.index + 1}</strong> — ${s.label}<br>
        <span class="save-slot-meta">W${s.wave} · ${s.difficulty} · ${when}</span></div>
  ...
  return {
    init, listSlots, saveToSlot, loadFromSlot, deleteSlot, renderPauseSlots, validateSlot,
    SLOT_COUNT,
  };
})();